...
module.exports = class extends Base {
  ...
  async detailAction() {
    ...
    orderInfo.province_name = await regionModel.where({id: orderInfo.province}).getField('name', true);
    orderInfo.city_name = await regionModel.where({id: orderInfo.city}).getField('name', true);
    orderInfo.district_name = await regionModel.where({id: orderInfo.district}).getField('name', true);
    orderInfo.full_region = orderInfo.province_name + orderInfo.city_name + orderInfo.district_name;

    orderInfo.express = await this.model('order_express').getLatestOrderExpress(orderId);
  ...
  async submitAction() {
    ...
    const actualPrice = orderTotalPrice - 0.00;
