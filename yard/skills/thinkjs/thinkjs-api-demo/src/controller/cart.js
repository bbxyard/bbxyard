...
module.exports = class extends Base {
  ...
  async addAction() {
    ...
    // 判断商品是否可以购买
    const goodsInfo = await this.model('goods').where({goods_id: goodsId}).find();
    if (think.isEmpty(goodsInfo) || goodsInfo.is_delete === 1) {
      return this.fail(400, '商品已下架');
    }

    // 取得规格的信息,判断规格库存
    const productInfo = await this.model('product').where({goods_id: goodsId, product_id: productId}).find();
    if (think.isEmpty(productInfo) || productInfo.number < number) {
    ...
    if (think.isEmpty(cartInfo)) {
      ...
      // 添加规格名和值
      let goodsSpecValue = [];
      if (!think.isEmpty(productInfo.goods_specifition_ids)) {
        goodsSpecValue = await this.model('goods_specification').where({
          goods_id: goodsId,
          id: {'in': productInfo.goods_specifition_ids.split('_')}
        }).getField('value');
      }
      ...
      const cartData = {
        ...
        goods_specifition_name_value: goodsSpecValue.join(';'),
        goods_specifition_ids: productInfo.goods_specifition_ids,
        checked: 1
      };
  ...
  async updateAction() {
    ...
    if (think.isEmpty(newCartInfo)) {
      ...
      if (!think.isEmpty(productInfo.goods_specification_ids)) {
        goodsSpec = await this.model('goods_specification').alias('gs')
          .field(['gs.*', 's.name'])
          .join({
            table: 'specification',
            join: 'inner',
            as: 's',
            on: ['gs.specification_id', 's.id']
          }).where('gs.goods_id': goodsId, 'gs.id': {'in': productInfo.goods_specification_ids.split('_')})
          .select();
      }

      const cartData = {
        number: number,
        goods_specifition_name_value: JSON.stringify(goodsSpec),
      ...
      const cartData = {
        ...
        goods_specifition_ids: newCartInfo.goods_specifition_ids,
        retail_price: productInfo.retail_price,
        market_price: productInfo.retail_price,
        product_id: productId,
        goods_sn: productInfo.goods_sn
  ...
  async goodsCountAction() {
    ...
    return this.success({
      cartTotal: { goodsCount: cartData.cartTotal.goodsCount }
    });
  }
